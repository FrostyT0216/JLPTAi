...
- 记录页：GitHub 风格每日正确率热力图（可切换年份，默认定位到最新日期）、累计天数/答题数/正确率/连续打卡、历史列表可回看试卷。
- 设置页：iOS 风格二级页面导航 —— 个性化（主题 / 背景：随机波动色块动画或 Bing 每日图片）、API 配置（服务商预设、测试连接）、数据管理（导出 / 导入 / 清空）。深色模式跟随系统。

## 数据存储（全部在本机 localStorage）

| Key | 内容 |
|---|---|
| `jlpt.settings.v1` | API 地址 / Key / 模型 / 温度 / JSON 开关 / 主题 / 背景 |
| `jlpt.bingbg.v1` | Bing 每日背景当日成功镜像源缓存 |
